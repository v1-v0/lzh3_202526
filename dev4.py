...
def create_simple_tagged_image(image_ch00, measurements_df, num_bacteria, save_path):
    ...
    ax.set_title(f'Tagged & Labeled Bacteria (Total: {num_bacteria})', 
...
def create_chart_images(measurements_df, image_ch00, image_ch01, labeled_bacteria):
    ...
    axes[0].set_title('Channel 00 (Brightfield)', fontweight='bold')
    axes[0].axis('off')
    ...
    axes[2].imshow(labeled_bacteria, cmap='nipy_spectral', alpha=0.5)
    axes[2].set_title('Detected Bacteria', fontweight='bold')
    axes[2].axis('off')
...
def export_to_excel_with_charts(measurements_df, excel_path, image_ch00, image_ch01, labeled_bacteria):
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        # Add images to summary sheet
        img_row = 12
        for key, buf in chart_images.items():
            img = XLImage(buf)
            img.width = 600
            img.height = int(img.height * (600 / img.width))
            summary_sheet.add_image(img, f'A{img_row}')
            img_row += int(img.height / 15) + 2
        
        # Sheet 2: All measurements
        measurements_df.to_excel(writer, sheet_name='All Measurements', index=False)
        
        # Format measurements sheet
        ws_measurements = writer.sheets['All Measurements']
        ...
        for idx in range(1, ws_measurements.max_column + 1):
            ...
            max_length = 0
            for row in range(1, ws_measurements.max_row + 1):
        ...
        # Sheet 4: Classification breakdown
        ...
        ws_class = writer.sheets['Classification Analysis']
        # Auto-adjust column widths for classification
        ...
        ws_top = writer.sheets['Top 20 by Intensity']
