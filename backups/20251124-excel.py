...
- Exports one worksheet per pair to Excel files grouped by source folder
...
- Output: outputs/<folder>_bacterial_analysis_results.xlsx (or bacterial_analysis_results.xlsx for root)
...
def export_to_excel(pairs: List[Tuple[Path, Path, str, str]]):
    """
    Export image pairs to Excel workbooks grouped by source folder.
    LANDSCAPE format with larger, more compact images.
    """
    folder_pairs = defaultdict(list)
    for bf_path, fluor_path, index, rel_path in pairs:
        folder = bf_path.parent
        folder_pairs[folder].append((bf_path, fluor_path, index, rel_path))

    print(f"\n{'='*70}")
    print(f"Starting Excel export (LANDSCAPE format, per folder)...")
    print(f"{'='*70}")

    for folder, pairs_in_folder in folder_pairs.items():
        if folder == INPUT_DIR:
            output_file = OUTPUT_DIR / "bacterial_analysis_results.xlsx"
        else:
            folder_name = folder.name
            output_file = OUTPUT_DIR / f"{folder_name}_bacterial_analysis_results.xlsx"
        ...
        for counter, (bf_path, fluor_path, global_index, rel_path) in enumerate(pairs_in_folder, 1):
            print(f"\n[{global_index}/{len(pairs):02d}] Processing: {rel_path}")
            ...
            bf_temp = save_temp_image(bf_rgb, f"bf_{global_index}")
            fluor_temp = save_temp_image(fluor_rgb, f"fluor_{global_index}")
            overlay_temp = save_temp_image(overlay_rgb, f"overlay_{global_index}")
            
            temp_files.extend([bf_temp, fluor_temp, overlay_temp])

            # Create worksheet
            print(f"  → Creating Excel worksheet (landscape layout)...")
            base_name = bf_path.name.replace('_ch00.tif', '')
            sheet_title = base_name[:31]  # Truncate to Excel sheet name limit
