...
class BacteriaReviewTool:
    ...
    def get_contour_intensities(self, image, labeled, props):
        ...
        for i, prop in enumerate(props):
            ...
            # Calculate statistics (only min and max as requested)
    ...
    def create_interface(self):
        ...
        # Create grid layout - reorganized to prevent overlap
        gs = self.fig.add_gridspec(6, 4, hspace=0.35, wspace=0.3, 
                                   left=0.05, right=0.95, top=0.96, bottom=0.25)
        ...
        # Stats text area (moved to top, expanded)
        self.ax_stats = self.fig.add_subplot(gs[3:6, :])
        self.ax_stats.axis('off')
        
        # Create sliders and controls (moved to bottom)
    ...
    def create_controls(self):
        ...
        # Controls positioned at the bottom of the figure
        slider_left = 0.08
        slider_width = 0.25
        slider_height = 0.015
        slider_spacing = 0.022
        
        # Base position from bottom
        base_y = 0.02
        ...
        # Min area slider - expanded range to 50-1000
        ...
        # Min fluor area slider - expanded range to 50-1000
        ...
        # Navigation buttons - positioned to the right of sliders
        button_width = 0.08
        button_height = 0.025
        button_x_start = slider_left + slider_width + 0.05
        button_y = base_y + 5*slider_spacing
        
        ax_prev = plt.axes((button_x_start, button_y, button_width, button_height))
        self.btn_prev = Button(ax_prev, 'Previous')
        self.btn_prev.on_clicked(self.prev_image)
        
        ax_next = plt.axes((button_x_start + button_width + 0.02, button_y, button_width, button_height))
        self.btn_next = Button(ax_next, 'Next')
        self.btn_next.on_clicked(self.next_image)
        
        ax_export = plt.axes((button_x_start + 2*(button_width + 0.02), button_y, button_width, button_height))
        self.btn_export = Button(ax_export, 'Export Config')
        self.btn_export.on_clicked(self.export_config)
        
        # Checkboxes for display options - positioned below buttons
        checkbox_x = button_x_start
        checkbox_y = base_y + 2*slider_spacing
        ax_checks = plt.axes((checkbox_x, checkbox_y, 0.20, 0.055))
    ...
    def update_statistics(self, gray_props, fluor_props, gray_contour_data, fluor_contour_data):
        ...
        if gray_contour_data:
            stats_lines.append(f"{'ID':<4} {'Area':<8} {'Mean Int':<10} {'Min Int':<9} {'Max Int':<9} {'Centroid (Y, X)':<20}")
            stats_lines.append("-" * 160)
