...
STREPTOCOCCUS_MITIS = SegmentationConfig(name='Streptococcus mitis',
    ...
    gaussian_sigma=12.0, min_area_um2=0.2, max_area_um2=50.0,
    min_aspect_ratio=0.8, max_aspect_ratio=6.0, min_circularity=0.5,
    max_circularity=1.0, dilate_iterations=1, erode_iterations=1,
    min_solidity=0.7)
DEFAULT = SegmentationConfig(name='Default (General Purpose)', description=
    'Generic bacteria detection profile', gaussian_sigma=15.0, min_area_um2
    =0.3, max_area_um2=2000.0)
_CONFIGS: Dict[str, SegmentationConfig] = {'proteus_mirabilis':
    PROTEUS_MIRABILIS, 'klebsiella_pneumoniae': KLEBSIELLA_PNEUMONIAE,
    'streptococcus_mitis': STREPTOCOCCUS_MITIS, 'default': DEFAULT}
bacteria_map = {'1': 'proteus_mirabilis', '2': 'klebsiella_pneumoniae', '3':
    'streptococcus_mitis', '4': 'default'}
bacteria_display_names = {'proteus_mirabilis': 'Proteus mirabilis',
    'klebsiella_pneumoniae': 'Klebsiella pneumoniae', 'streptococcus_mitis':
    'Streptococcus mitis', 'default': 'Default (General Purpose)'}
...
def print_config_comparison(configs: list[SegmentationConfig]) ->None:
    ...
    print()
    print('-' * 80)
    params = [('Gaussian σ', 'gaussian_sigma'), ('Min Area (µm²)',
        'min_area_um2'), ('Max Area (µm²)', 'max_area_um2'), (
        'Min Aspect Ratio', 'min_aspect_ratio'), ('Max Aspect Ratio',
        'max_aspect_ratio'), ('Min Circularity', 'min_circularity'), (
        'Dilate Iterations', 'dilate_iterations'), ('Erode Iterations',
        'erode_iterations')]
...
def load_config_from_json(json_path: Path) ->SegmentationConfig:
    ...
    try:
        ...
        um2_per_px2 = 0.012
        config = SegmentationConfig(name=
            f'{bacterium} ({structure}, {mode}) [JSON]', description=
            f'Parameters loaded from {json_path.name}', gaussian_sigma=
            float(params.get('gaussian_sigma', 15.0)), min_area_um2=float(
            params.get('min_area', 240.0)) * um2_per_px2, max_area_um2=
            float(params.get('max_area', 7333.0)) * um2_per_px2,
            dilate_iterations=int(params.get('dilate_iterations', 1)),
            erode_iterations=int(params.get('erode_iterations', 1)),
            morph_kernel_size=3, morph_iterations=1, min_circularity=0.0,
            max_circularity=1.0, min_aspect_ratio=0.2, max_aspect_ratio=
            10.0, min_mean_intensity=0, max_mean_intensity=255,
            max_edge_gradient=200, min_solidity=0.3, max_fraction_of_image=
            0.25, fluor_min_area_um2=3.0, fluor_match_min_intersection_px=5.0)
        print(f'  ✓ Loaded JSON parameters:')
        print(f'    • Bacterium: {bacterium}')
        print(f'    • Gaussian σ: {config.gaussian_sigma:.2f}')
        print(
            f'    • Size range: {config.min_area_um2:.2f} - {config.max_area_um2:.2f} µm²'
            )
        return config
...
bacteria_configs = {'Klebsiella Pneumoniae': {'bacteria_segmentation': {
    'invert_image': False, 'gaussian_sigma': 4.1, 'brightness_adjust': 70,
    'contrast_adjust': 2.0, 'threshold_offset': -40, 'min_area': 240.31,
    'max_area': 7332.94, 'dilate_iterations': 0, 'erode_iterations': 0},
    'inclusion_dark_segmentation': {'invert_image': False, 'gaussian_sigma':
    2.0, 'brightness_adjust': 0, 'contrast_adjust': 1.0, 'threshold_offset':
    -10, 'min_area': 10, 'max_area': 2000, 'dilate_iterations': 1,
    'erode_iterations': 1}, 'inclusion_bright_segmentation': {
    'invert_image': True, 'gaussian_sigma': 2.0, 'brightness_adjust': 0,
    'contrast_adjust': 1.0, 'threshold_offset': -10, 'min_area': 10,
    'max_area': 2000, 'dilate_iterations': 1, 'erode_iterations': 1}}}
if __name__ == '__main__':
    ...
    for bacteria_type in list_available_configs():
        ...
        print(
            f'    σ={config.gaussian_sigma:.1f}, Size={config.min_area_um2:.1f}-{config.max_area_um2:.1f} µm²'
            )
    print('\n')
    configs_to_compare = [get_config('proteus_mirabilis'), get_config(
        'klebsiella_pneumoniae'), get_config('streptococcus_mitis')]
