...
def export_to_excel_with_charts(measurements_df, excel_path, image_ch00, image_ch01, labeled_bacteria):
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        # Set to 150% of original size
        img.width = int(img.width * 1.10)   # ← 110% width
        img.height = int(img.height * 1.10)  # ← 110% height
        ...
        for idx in range(1, ws_measurements.max_column + 1):
            ...
            max_length = 0
            for row in range(1, min(ws_measurements.max_row + 1, 100)):  # Check first 100 rows for performance
