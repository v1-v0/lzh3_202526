...
class SegmentationViewer:
    ...
    def navigate_to_bacteria(self, index: int):
        ...
        pixel_value = int(self.original_image[cy, cx]) if self.original_image is not None else 0
        ...
        self.status_var.set(f"Viewing bacterium {index + 1}/{len(self.bacteria_stats)} - "
                           f"Area: {stat['bf_area']:.1f} px², Fluor: {stat['fluor_total']:.1f}")
    ...
    def setup_ui(self):
        ...
        # Row 2: Previous and Next (no counter in between)
    ...
    # --------------------------------------------------------------------- #
    # NEW: Update compact statistics panel
    # --------------------------------------------------------------------- #
    def update_compact_statistics(self):
        """Update the compact statistics panel with overview data only."""
        if self.bacteria_stats:
            # Always show overview statistics (no individual bacterium details)
    ...
    def setup_statistics_table(self):
        ...
        columns = ("ID", "BF Area (px²)", "Fluor Mean", "Fluor Total", "Fluor/Area")
        ...
        self.stats_tree.column("ID", width=60, anchor=tk.CENTER)
    ...
    def draw_labels_on_contours(self, img_bgr, contours):
        ...
        for idx, contour in enumerate(contours, 1):
            ...
            is_selected = (self.current_bacteria_index == idx - 1)
            arrow_color = (255, 128, 0) if is_selected else (255, 255, 0)  # Orange if selected, yellow otherwise
    ...
    def update_preview(self) -> None:
        ...
        try:
            ...
            self.bacteria_stats = self.calculate_bacteria_statistics(
                bacteria, 
                self.original_image, 
    ...
    def exit_application(self):
        ...
        try:
            ...
            for canvas in [self.canvas_original, self.canvas_fluorescence, self.canvas_enhanced,
                           ...
                           self.canvas_contours, self.canvas_overlay]:
                canvas.delete("all")
                # cast to Any to satisfy the type checker and safely remove the attribute if present
                c = cast(Any, canvas)
