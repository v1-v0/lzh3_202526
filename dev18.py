...
# ==================================================
# Multi-scan → Clinical follow-up helpers
# ==================================================

# ==================================================
# Missing Helper Functions for main()
# ==================================================
...
def configure_dataset() -> dict:
    """Collect dataset configuration from user"""
    config = {}
    ...
    if has_gplus and has_gminus:
        ...
        config['dataset_id'] = source_dir.name
    else:
        config['batch_mode'] = False
    
    # Get percentile
    print("\n" + "─" * 80)
    print("Percentile Configuration")
    print("─" * 80)
    ...
    # Get threshold
    print("\n" + "─" * 80)
    print("Clinical Threshold Configuration")
    print("─" * 80)
...
def validate_config(config: dict) -> bool:
    ...
    required_keys = ['source_dir', 'batch_mode', 'percentile', 'threshold_pct']
    ...
    # ✅ FIX: Check source_dir only once
    source_dir = config.get('source_dir')
    if source_dir is None:
        print(f"[ERROR] source_dir is None")
        return False
    ...
    # ✅ FIX: Add return True - validation passed!
    return True
...
def generate_final_clinical_matrix_wrapper(output_root: Path, config: dict) -> None:
    """Generate final clinical decision matrix combining G+ and G-"""
    
    print("\n" + "=" * 80)
    print("GENERATING FINAL CLINICAL MATRIX")
    print("=" * 80)
    
    # Load G+ classification
    ...
    # Generate matrix
    dataset_base = config.get('dataset_id', 'Dataset')
...
def cleanup_and_reorganize_output(output_root: Path, config: dict) -> None:
    ...
    try:
        ...
        if positive_dir and positive_dir.exists():
            ...
            # ✅ Copy confidence report from Positive/ (if it exists there)
        ...
        files_to_remove = [
            "configuration_comparison.csv",  # Redundant with confidence_report.txt
            # "confidence_report.txt",  # ✅ REMOVED FROM DELETION LIST
        ...
        if removed_files == 0:
            print(f"  (No unnecessary files found)")
        
        # ========== STEP 3: Remove Configuration Folders ==========
        print("\n🗑️  Removing configuration scan folders...")
        ...
        if removed_dirs == 0:
            print(f"  (No configuration folders found)")
...
def _get_best_match_config_key(multi_scan_result: dict) -> Optional[str]:
     """Extract Config_Key from a multi-scan result dict returned by process_multi_configuration."""
     try:
         best = multi_scan_result.get("best_match")
         if best is None:
             return None
         # best_match is a pandas Series row in your code, but dict-like access works
         key = best.get("Config_Key")  # type: ignore[attr-defined]
         return str(key) if key is not None else None
     except Exception:
         return None

def _print_clinical_followup_banner(chosen_key: str, reason: str) -> None:
     print("\n" + "=" * 80)
     print("AUTO CLINICAL FOLLOW-UP (POST MULTI-SCAN)")
     print("=" * 80)
     print(f"Chosen bacteria configuration: {chosen_key}")
...
# ==================================================
# process_multi_configuration with Statistics
# ==================================================
...
def process_multi_configuration(config: dict) -> dict:
    ...
    """
    # Extract from bacteria_config_info
    bacteria_config_info = config.get('bacteria_config_info', {})
    ...
    # ✅ FIX: Use 'current_source' (points to G+ or G-) instead of 'source_dir' (parent)
    ...
    for bacteria_type in configs_to_scan:
        ...
        # Initialize results with per-image tracking
        scan_results = {
                    ...
                    try:
                        ...
                        if csv_path.exists():
                            ...
                            if 'Fluor_Mean' in df.columns and len(df) > 0:
                                fluor_values = pd.to_numeric(df['Fluor_Mean'], errors='coerce')
                                valid_fluor = fluor_values.dropna()

                                if len(valid_fluor) > 0:
                            ...
                            save_cache(cache_dir, cache_key, cache_data)
        ...
        if scan_results['images_processed'] > 0:
            # Mean fluorescence
            scan_results['mean_fluorescence'] = scan_results['fluorescence_sum'] / scan_results['images_processed']
            
            # Standard deviation and SEM for fluorescence
            if len(scan_results['per_image_fluorescence']) > 1:
                fluor_array = np.array(scan_results['per_image_fluorescence'])
                scan_results['std_fluorescence'] = float(np.std(fluor_array, ddof=1))
                scan_results['sem_fluorescence'] = float(scipy_stats.sem(fluor_array))
            else:
            ...
            if len(scan_results['per_image_particles']) > 1:
                particle_array = np.array(scan_results['per_image_particles'])
                scan_results['mean_particles_per_image'] = float(np.mean(particle_array))
                scan_results['std_particles'] = float(np.std(particle_array, ddof=1))
            else:
                scan_results['mean_particles_per_image'] = float(scan_results['particles_detected'])
        ...
        # Consolidate to Excel (if images were processed)
        if scan_results['images_processed'] > 0:
            for group_name in image_groups.keys():
        ...
        print(f"    Particles per image: {scan_results['mean_particles_per_image']:.1f} ± {scan_results['std_particles']:.1f}")
        print(f"    Mean fluorescence: {scan_results['mean_fluorescence']:.2f} ± {scan_results['std_fluorescence']:.2f} a.u./µm²")
    ...
    print(f"   Hit rate: {cache_hits / (cache_hits + cache_misses) * 100:.1f}%" if (cache_hits + cache_misses) > 0 else "   Hit rate: N/A")
    
    # Generate comparison and confidence report
...
# ==================================================
# generate_confidence_report with Statistics
# ==================================================
...
def generate_confidence_report(
    ...
    """Generate confidence report with statistical analysis and error bars
    
    Args:
        all_results: Dictionary of results from each configuration
        config_names: Mapping of config keys to display names
        output_dir: Output directory
        
    Returns:
        tuple: (comparison DataFrame with statistics, formatted report string)
    """
    ...
    for bacteria_type, results in all_results.items():
        ...
        comparison_data.append({
            ...
            'Confidence_Percent': float(confidence_score)  # For sorting
        })
    
    # Sort by confidence
    comparison_df = pd.DataFrame(comparison_data)
    comparison_df = comparison_df.sort_values('Confidence_Percent', ascending=False)
    comparison_df['Rank'] = range(1, len(comparison_df) + 1)
    
    # Reorder columns
    comparison_df = comparison_df[[
        'Rank', 'Bacteria_Type', 'Config_Key', 'Particles_Detected',
        'Particles_Per_Image', 'Particles_Std', 'Mean_Fluorescence', 
        'Fluor_Std', 'Fluor_SEM', 'Confidence_Score', 'Images_Processed', 
        'Images_Failed', 'Confidence_Percent'
    ]]
    ...
    if top_particles == 0:
        ...
        report_lines.append("")
        report_lines.append(f"✓ Strong match to: {top_result['Bacteria_Type']}")
    elif top_confidence >= 50:
        report_lines.append("Status: BACTERIA DETECTED")
        report_lines.append(f"Confidence: Moderate ({top_confidence:.1f}%)")
        report_lines.append("")
        report_lines.append(f"⚠ Possible match to: {top_result['Bacteria_Type']}")
        ...
        report_lines.append("   → Multiple configurations show similar results")
        report_lines.append("   → Consider culture-based confirmation")
    
    report_lines.append("")
    report_lines.append("")
    
    # Top 3 matches with statistics
    ...
    for idx in range(min(3, len(comparison_df))):
        ...
        particles = int(row['Particles_Detected'])
        particles_per_img = float(row['Particles_Per_Image'])
        particles_std = float(row['Particles_Std'])
        ...
        # Format fluorescence display
        if pd.isna(fluor_mean) or fluor_mean == 0:
            fluor_display = "N/A"
        else:
            fluor_display = f"{fluor_mean:.2f} ± {fluor_std:.2f} a.u./µm² (SEM: ±{fluor_sem:.2f})"
        ...
        report_lines.append(f"   Particles/image: {particles_per_img:.1f} ± {particles_std:.1f}")
        report_lines.append(f"   Mean fluorescence: {fluor_display}")
        report_lines.append("")
    ...
    display_df['Particles_Per_Image'] = display_df['Particles_Per_Image'].apply(lambda x: f"{float(x):.1f}")
    display_df['Particles_Std'] = display_df['Particles_Std'].apply(lambda x: f"±{float(x):.1f}")
    display_df['Mean_Fluorescence'] = display_df['Mean_Fluorescence'].apply(lambda x: f"{float(x):.2f}")
    display_df['Fluor_Std'] = display_df['Fluor_Std'].apply(lambda x: f"±{float(x):.2f}")
    display_df['Fluor_SEM'] = display_df['Fluor_SEM'].apply(lambda x: f"±{float(x):.2f}")
    display_df['Confidence_Score'] = display_df['Confidence_Score'].apply(lambda x: f"{float(x):.1f}")
    
    display_cols = [
        'Rank', 'Bacteria_Type', 'Particles_Detected', 'Particles_Per_Image',
        'Particles_Std', 'Mean_Fluorescence', 'Fluor_Std', 'Fluor_SEM',
    ...
    if len(all_results) >= 2:
        ...
        # Get top 2 configurations
        config_keys = list(comparison_df['Config_Key'].values)
        top1_key = str(config_keys[0])
        top2_key = str(config_keys[1])
        
        top1_name = str(config_names[top1_key])
        top2_name = str(config_names[top2_key])
        
        # Get per-image data
        top1_fluor = all_results[top1_key].get('per_image_fluorescence', [])
        top2_fluor = all_results[top2_key].get('per_image_fluorescence', [])
        
        # Convert to float arrays (in case they're not)
        top1_fluor = [float(x) for x in top1_fluor if x is not None and not pd.isna(x)]
        top2_fluor = [float(x) for x in top2_fluor if x is not None and not pd.isna(x)]
        
        if len(top1_fluor) >= 3 and len(top2_fluor) >= 3:
            # Perform t-test
            try:
                # ✅ FIXED: Explicitly unpack and convert to float
                ...
                report_lines.append(f"Comparison: {top1_name} vs {top2_name}")
                report_lines.append(f"  Independent t-test (Welch's):")
                report_lines.append(f"    t-statistic: {t_stat:.4f}")
                report_lines.append(f"    p-value: {p_value:.6f}")
                
                # ✅ FIXED: Now comparing float to float
                if p_value < 0.001:
                ...
                if p_value < 0.05:
                    report_lines.append(f"  ✓ Top configuration ({top1_name}) is statistically distinguishable")
                else:
                    report_lines.append(f"  ⚠ Top configurations are NOT statistically different")
                    report_lines.append(f"    → Consider both as viable candidates")
            ...
            report_lines.append("  Insufficient data for statistical testing (need ≥3 images per config)")
            report_lines.append(f"    Top 1 has {len(top1_fluor)} images, Top 2 has {len(top2_fluor)} images")
    ...
    if top_particles == 0:
        report_lines.append("✓ No bacteria detected across all configurations")
        report_lines.append("  → Sample appears clean")
        report_lines.append("  → No immediate action required")
    elif top_confidence >= 70:
        report_lines.append(f"✓ High confidence match: {top_result['Bacteria_Type']}")
        report_lines.append("  → Proceed with targeted antimicrobial therapy")
        report_lines.append("  → Consider culture confirmation if treatment fails")
    elif top_confidence >= 50:
        report_lines.append(f"⚠ Moderate confidence match: {top_result['Bacteria_Type']}")
        report_lines.append("  → Recommend culture-based confirmation")
        report_lines.append("  → Consider broad-spectrum coverage initially")
    else:
        report_lines.append("⚠ LOW CONFIDENCE - MANUAL REVIEW REQUIRED")
        report_lines.append("  → Multiple configurations show similar results")
        report_lines.append("  → STRONGLY recommend culture-based identification")
        report_lines.append("  → Consider infectious disease consultation")
...
# ==================================================
# calculate_confidence_score (Improved Algorithm)
# ==================================================
...
def calculate_confidence_score(results: dict) -> float:
    ...
    Scoring Logic:
    - Prioritize HIGH fluorescence signal (quality over quantity)
    - Reward moderate particle counts (not too many, not too few)
    - Penalize excessive detections (likely noise)
    
    Args:
        results: Results dictionary with detection metrics
    ...
    # Handle NaN/None
    if mean_fluor is None or pd.isna(mean_fluor):
        mean_fluor = 0.0
    
    # ========== FACTOR 1: Fluorescence Quality (0-50 points) ==========
    # This is now the PRIMARY factor - high signal = high confidence
    if mean_fluor >= 75:
        fluor_score = 50  # Excellent signal
    elif mean_fluor >= 50:
        fluor_score = 45  # Very good signal
    elif mean_fluor >= 30:
        fluor_score = 40  # Good signal
    elif mean_fluor >= 20:
        fluor_score = 30  # Moderate signal
    elif mean_fluor >= 10:
        fluor_score = 20  # Low signal
    elif mean_fluor >= 5:
        fluor_score = 10  # Very low signal
    elif mean_fluor > 0:
        fluor_score = 5   # Minimal signal
    else:
        fluor_score = 0   # No signal
    
    score += fluor_score
    
    # ========== FACTOR 2: Particle Count Appropriateness (0-30 points) ==========
    # Reward MODERATE counts, penalize extreme counts
    if 20 <= n_particles <= 200:
        # Sweet spot - specific detection
        particle_score = 30
    elif 10 <= n_particles < 20:
        # Low but acceptable
        particle_score = 25
    elif 200 < n_particles <= 1000:
        # Getting noisy
        particle_score = 15
    elif 1000 < n_particles <= 5000:
        # Very noisy - probably detecting crystals/artifacts
        particle_score = 5
    ...
    # ========== FACTOR 3: Processing Success Rate (0-10 points) ==========
    total_images = results['images_processed'] + results['images_failed']
    if total_images > 0:
        success_rate = results['images_processed'] / total_images
        score += success_rate * 10
    
    # ========== FACTOR 4: Quality Bonus (0-10 points) ==========
    # Reward high fluorescence with appropriate particle count
    if mean_fluor >= 30 and 10 <= n_particles <= 200:
        score += 10  # High quality, specific detection
    elif mean_fluor >= 20 and 20 <= n_particles <= 500:
...
def generate_final_clinical_matrix(
    ...
    ✅ FIXED: Now correctly handles Control group in final matrix
    
    Args:
        output_root: Root output directory
        gplus_classification: G+ classification DataFrame
        gminus_classification: G- classification DataFrame
        dataset_base_name: Base dataset name
    ...
    for group in all_groups:
        ...
        if gplus_data:
            try:
                gplus_mean = float(gplus_data.get('Mean', 0))
                gplus_class = str(gplus_data.get('Classification', 'MISSING'))
            except (TypeError, ValueError):
                gplus_mean = None
                gplus_class = 'INVALID DATA'
        else:
            gplus_mean = None
            gplus_class = None
        ...
        if gminus_data:
            try:
                gminus_mean = float(gminus_data.get('Mean', 0))
                gminus_class = str(gminus_data.get('Classification', 'MISSING'))
            except (TypeError, ValueError):
                gminus_mean = None
                gminus_class = 'INVALID DATA'
        else:
            gminus_mean = None
            gminus_class = None
        
        # ✅ FIX: Extract detection status from classification string
        def extract_detection_status(classification):
            """Extract simple detection status from classification string"""
            if classification is None:
                return 'N/A'
            if 'CONTROL' in classification.upper():
                return 'Control'
            if 'POSITIVE' in classification and 'No obvious' not in classification:
                return 'Detected'
            if 'NEGATIVE' in classification and 'No obvious' not in classification:
                return 'Detected'
            if 'No obvious' in classification:
                return 'Not Detected'
            return 'Unknown'
        
        gplus_detection = extract_detection_status(gplus_class)
        gminus_detection = extract_detection_status(gminus_class)
    ...
    try:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            control_fill = PatternFill(start_color="E7E6E6", end_color="E7E6E6", fill_type="solid")
            ...
            legend_data = pd.DataFrame({
                'Classification': [
                    'POSITIVE', 'NEGATIVE', 'NO OBVIOUS BACTERIA', 
...
def display_configuration_summary(config: dict) -> None:
    ...
    mode = config.get('bacteria_mode', 'single')
    bacteria_config_info = config.get('bacteria_config_info', {})
    ...
    if mode == 'multi_scan':
        ...
        configs_to_scan = bacteria_config_info.get('configs_to_scan', [])
        config_names = bacteria_config_info.get('config_names', {})
        print(f"   Configurations to test: {len(configs_to_scan)}")
        for cfg_key in configs_to_scan[:5]:  # Show first 5
            print(f"     • {config_names.get(cfg_key, cfg_key)}")
        if len(configs_to_scan) > 5:
            print(f"     ... and {len(configs_to_scan) - 5} more")
        print()
    else:
        bacteria_type = config.get('bacteria_type', 'unknown')
        bacteria_config = load_bacteria_config_from_json(bacteria_type)
        if bacteria_config:
            print(f"Processing Mode: SINGLE CONFIGURATION")
            print(f"   Bacteria: {bacteria_config.name}")
            print(f"   Description: {bacteria_config.description}")
            print(f"   Gaussian σ: {bacteria_config.gaussian_sigma}")
            print(f"   Area range: {bacteria_config.min_area_um2} - {bacteria_config.max_area_um2} µm²")
            print()
    
    # Rest of summary (unchanged)
    if config.get('batch_mode', False):
        print(f"Dataset Mode: BATCH PROCESSING")
        ...
        print(f"Dataset Mode: SINGLE PROCESSING")
        print(f"1. Source Directory: {config['source_dir']}")
        print(f"2. Dataset ID: {config['dataset_id']}")
        print(f"3. Percentile: {config['percentile']*100:.0f}%")
        print(f"4. Microgel Type: {config.get('microgel_type', 'N/A').capitalize()}")
...
def main():
    ...
    try:
        ...
        # ✅ ADDED: Validate batch structure if in batch mode
        if config.get('batch_mode'):
            if not validate_batch_structure(config):
                print("❌ Batch structure validation failed")
        ...
        if config['batch_mode']:
            parent_name = f"{config['timestamp']}_{config['dataset_id']}"
            output_root = OUTPUTS_DIR / parent_name
            output_root.mkdir(parents=True, exist_ok=True)
            
            config['positive_output'] = output_root / "Positive"
            config['negative_output'] = output_root / "Negative"
            config['positive_output'].mkdir(exist_ok=True)
            config['negative_output'].mkdir(exist_ok=True)
            ...
            dir_name = f"{config['timestamp']}_{config['dataset_id']}"
