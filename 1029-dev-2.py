...
- Preserves RED color in fluorescence channel
- Excel + 3 PNGs: BF+contours, FLUO (red)+contours, Overlay (BF + red FLUO)
"""
...
def save_visualisations(bf_gray, fluo_color, bacteria, base_name):
    ...
    # 2. Fluorescence (RED) + green contours
    ...
    # 3. Overlay: BF (60%) + RED FLUO (40%)
    # Extract red channel and make pseudo-color
    red_channel = fluo_color[:, :, 2]  # BGR → R is index 2
    red_pseudo = np.zeros_like(fluo_color)
    red_pseudo[:, :, 2] = red_channel  # Only red channel
...
def main():
    ...
    for bf_path, fluo_path in pairs:
        ...
        # Load brightfield as grayscale (for segmentation)
        bf_gray   = load_image(bf_path, gray=True)

        # Load fluorescence in COLOR (to preserve red)
        fluo_color = load_image(fluo_path, gray=False)  # ← BGR

        # For measurement: use grayscale version
        fluo_gray = cv2.cvtColor(fluo_color, cv2.COLOR_BGR2GRAY)

        # Align using grayscale versions
        fluo_gray_aligned, _ = align_ecc(bf_gray, fluo_gray)

        # But warp the COLOR image for visualization
        _, warp_matrix = align_ecc(bf_gray, fluo_gray)
        fluo_color_aligned = cv2.warpPerspective(
            fluo_color, warp_matrix,
            (bf_gray.shape[1], bf_gray.shape[0]),
            flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP
        )
        ...
        # Measure using aligned grayscale
        ...
        # Save visualizations with RED fluorescence
