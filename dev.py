...
def create_summary_plots(all_data, non_matching_data, dataset_name, output_dir):
...
def main():
    ...
    try:
        ...
        source_base = Path(CONFIG["source_folder"][0])
        ...
        if not subdirs:
            print(f"⚠ No subdirectories found in {source_base}")
            print(f"  Processing images directly from {source_base}...")
            subdirs = [source_base]
        else:
            print(f"\nFound {len(subdirs)} folder(s) to process: {[d.name for d in subdirs]}\n")
        
        # Process each subdirectory
        for subdir in subdirs:
            dataset_name = subdir.name
            print(f"\n{'='*70}")
            print(f"PROCESSING FOLDER: {dataset_name}")
            print(f"{'='*70}\n")
            
            # Temporarily update config to point to current subdirectory
            original_source = CONFIG["source_folder"][0]
            CONFIG["source_folder"][0] = str(subdir)
            
            # Setup output folder for this dataset
            base_output = Path(CONFIG["output_folder"][0])
            ...
            # Load image pairs for this folder
            try:
                gray_images, fluor_images, image_paths = load_image_pairs()
            except Exception as e:
                print(f"⚠ Error loading images from {dataset_name}: {e}")
                CONFIG["source_folder"][0] = original_source  # Restore original
                continue
            
            total_images = len(gray_images)
            
            if total_images == 0:
                print(f"⚠ No image pairs found in {dataset_name}")
                CONFIG["source_folder"][0] = original_source  # Restore original
                continue
            ...
            print(f"\n{'='*70}")
            print(f"PROCESSING {total_images} IMAGE PAIR(S) FROM {dataset_name}")
            print(f"{'='*70}\n")
            ...
            for idx, (gray_img, fluor_img, image_path) in enumerate(zip(gray_images, fluor_images, image_paths), 1):
                try:
                    ...
                    output_filename = output_dir / f"{prefix}_dual_channel_{idx}.png"
            ...
            if all_data:
                ...
                create_summary_plots(all_data, all_non_matching, dataset_name, output_dir)
                ...
                print(f"⚠ No bacteria detected in {dataset_name}. No CSV or plots generated.\n")
            
            # Folder summary
            print(f"{'='*70}")
            print(f"FOLDER {dataset_name} COMPLETE")
            print(f"{'='*70}")
            ...
            CONFIG["source_folder"][0] = original_source
