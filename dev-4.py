...
- ✨ NEW: Plan B - Global display controls above tabs
"""
...
class SegmentationViewer:
    def __init__(self, root):
        ...
        # ✨ NEW: Global display control variables
        self.global_show_contours = tk.BooleanVar(value=True)
        self.global_show_labels = tk.BooleanVar(value=True)
        self.global_show_scale = tk.BooleanVar(value=True)
    ...
    # --------------------------------------------------------------------- #
    # ✨ NEW: Global display controls
    # --------------------------------------------------------------------- #
    def create_global_display_controls(self, parent):
        """Create global display controls above the notebook tabs."""
        control_frame = ttk.Frame(parent, relief=tk.RIDGE, borderwidth=1, padding=8)
        control_frame.pack(side=tk.TOP, fill=tk.X, padx=0, pady=(0, 5))
        
        ttk.Label(control_frame, text="Display Controls:", 
                 font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT, padx=(0, 15))
        
        cb_contours = ttk.Checkbutton(
            control_frame,
            text="Show Contours",
            variable=self.global_show_contours,
            command=self.update_preview
        )
        cb_contours.pack(side=tk.LEFT, padx=8)
        ToolTip(cb_contours, "Toggle contour outlines on applicable tabs (Contours, Overlay)")
        
        cb_labels = ttk.Checkbutton(
            control_frame,
            text="Show Labels",
            variable=self.global_show_labels,
            command=self.update_preview
        )
        cb_labels.pack(side=tk.LEFT, padx=8)
        ToolTip(cb_labels, "Toggle numbered bacteria labels on Contours tab")
        
        cb_scale = ttk.Checkbutton(
            control_frame,
            text="Show Scale Bar",
            variable=self.global_show_scale,
            command=self.update_preview
        )
        cb_scale.pack(side=tk.LEFT, padx=8)
        ToolTip(cb_scale, "Toggle scale bar with physical units (µm) on all applicable tabs")
        ...
        hint_label = ttk.Label(
            control_frame,
            text="💡 Global controls affect: Original, Enhanced, Contours, Overlay, Fluorescence",
            font=("Segoe UI", 8),
            foreground="#888"
        )
    ...
    # --------------------------------------------------------------------- #
    # UI construction (✨ UPDATED with global controls)
    # --------------------------------------------------------------------- #
    def setup_ui(self):
        ...
        # ---- Image tabs with global controls (✨ NEW) -------------------
        img_frame = ttk.Frame(main)
        img_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # ✨ Global display controls above tabs
        self.create_global_display_controls(img_frame)
    ...
    # --------------------------------------------------------------------- #
    # Statistics table (WITH HISTOGRAM)
    # --------------------------------------------------------------------- #
    def setup_statistics_table(self):
        ...
        # Bottom section: Histogram (only if matplotlib available)
    ...
    # --------------------------------------------------------------------- #
    # Scale bar (✨ UPDATED to check global setting)
    # --------------------------------------------------------------------- #
    def draw_scale_bar(self, img_pil, pixel_size_um):
        """Draw scale bar on PIL image with physical units (if globally enabled)."""
        if not self.global_show_scale.get():
            return img_pil
    ...
    def create_overlay_image(self, bf, fluor, contours):
        ...
        if self.global_show_contours.get():
    ...
    def create_label_controls(self, parent):
        ...
        lf = ttk.LabelFrame(parent, text=" Label Styling ", padding=8)
        lf.pack(fill=tk.X, pady=(0, 5))
    ...
    def reset_to_defaults(self):
        ...
        if messagebox.askyesno("Reset", "Restore all defaults?"):
            ...
            # Reset global display controls
            self.global_show_contours.set(True)
            self.global_show_labels.set(True)
            self.global_show_scale.set(True)
    ...
    def draw_labels_on_contours(self, img_bgr, contours):
        """Draw numbered labels on bacteria contours with smart positioning (✨ UPDATED)."""
        # ✨ Check global setting
        if not self.global_show_labels.get():
            return img_bgr
    ...
    # --------------------------------------------------------------------- #
    # Preview update (✨ UPDATED with global controls)
    # --------------------------------------------------------------------- #
    ...
    def _do_update_preview(self):
        """Actual preview update logic (✨ UPDATED for global controls)."""
        try:
            if self.original_image is None:
                return
            ...
            # Tab 1: Original - with optional scale bar
            orig_rgb = cv2.cvtColor(self.original_image, cv2.COLOR_GRAY2RGB)
            orig_pil = Image.fromarray(orig_rgb)
            orig_pil = self.draw_scale_bar(orig_pil, pixel_size)
            orig_with_scale = cv2.cvtColor(np.array(orig_pil), cv2.COLOR_RGB2BGR)
            self.display_image(orig_with_scale, self.canvas_original)

            # Tab 2: CLAHE Enhanced - with optional scale bar
            enh_rgb = cv2.cvtColor(enhanced, cv2.COLOR_GRAY2RGB)
            enh_pil = Image.fromarray(enh_rgb)
            enh_pil = self.draw_scale_bar(enh_pil, pixel_size)
            enh_with_scale = cv2.cvtColor(np.array(enh_pil), cv2.COLOR_RGB2BGR)
            self.display_image(enh_with_scale, self.canvas_enhanced)
            ...
            # Tab 5: Final Contours - with contours, labels, and scale bar
            ...
            # Draw labels if enabled (global setting checked inside function)
            cnt_img = self.draw_labels_on_contours(cnt_img, bacteria)
            
            # Draw scale bar if enabled
            cnt_rgb = cv2.cvtColor(cnt_img, cv2.COLOR_BGR2RGB)
            cnt_pil = Image.fromarray(cnt_rgb)
            cnt_pil = self.draw_scale_bar(cnt_pil, pixel_size)
            cnt_img = cv2.cvtColor(np.array(cnt_pil), cv2.COLOR_RGB2BGR)
            
            self.display_image(cnt_img, self.canvas_contours)

            # Tab 6: Overlay - uses create_overlay_image which checks global_show_contours
            ...
            if self.fluorescence_image is not None:
                ...
                fluor_pil = Image.fromarray(cv2.cvtColor(red, cv2.COLOR_BGR2RGB))
                fluor_pil = self.draw_scale_bar(fluor_pil, pixel_size)
                fluor_with_scale = cv2.cvtColor(np.array(fluor_pil), cv2.COLOR_RGB2BGR)
                
                self.display_image(fluor_with_scale, self.canvas_fluorescence)
            else:
