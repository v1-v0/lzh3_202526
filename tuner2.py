# feedback_tuner.py - Enhanced version with dev2a.py integration
...
# ==================================================
# Unicode-Safe File I/O Functions (from dev2a.py)
# ==================================================
...
# ==================================================
# Metadata Extraction (from dev2a.py)
# ==================================================
...
def normalize_to_8bit(img: np.ndarray) -> np.ndarray:
    """Normalize image to 8-bit (from dev2a.py)"""
...
# ==================================================
# Enhanced SegmentationTuner Class
# ==================================================

class SegmentationTuner:
    """Interactive segmentation parameter tuner with dev2a.py integration"""
    ...
    def _load_image(self, image_path: Path) -> np.ndarray:
        """Load and validate image with Unicode support"""
    ...
    def _create_header(self, parent: ttk.Frame):
        """Create header section with metadata info"""
    ...
    def _create_parameters_section(self, parent: ttk.Frame):
        """Create parameters display section with metadata info"""
        # Header
        ...
        # Filtering - show both pixels and micrometers
    ...
    def process_image(self):
        """Process image with current parameters (enhanced from dev2a.py)"""
    ...
    def _update_histogram(self):
        """Update area distribution histogram with µm² units"""
        self.ax_hist.clear()
    ...
    def _update_param_displays(self):
        """Update parameter display values with µm² conversion"""
        # Update invert
    ...
    def _analyze_particle(self, contour: np.ndarray):
        ...
        for param, value in suggestions.items():
            # Convert area suggestions to show both px and µm²
            if 'area' in param:
        ...
        print(f"\n🎯 Particle Analysis:")
        print(f"   Area: {area_px:.1f} px² ({area_um2:.2f} µm²)")
    ...
    def load_new_image(self):
        """Load a new image file with Unicode support"""
        ...
        try:
            ...
            # Update header
            self._update_header_after_load()
    ...
    def _update_header_after_load(self):
        """Update header badges after loading new image"""
        # This is called after loading a new image
        # The header should reflect the new metadata status
        pass
    
    def save(self, event=None) -> bool:
        """Save parameters to JSON with metadata info"""
        try:
            config = {
    ...
    def export_complete_config(self):
        ...
        complete_config = {
            ...
            "statistics": {
                ...
                "total_area_um2": sum(a * um2_per_px2 for a in self.contour_areas) if self.contour_areas else 0,
                "mean_area_um2": (sum(a * um2_per_px2 for a in self.contour_areas) / len(self.contour_areas)) if self.contour_areas else 0,
            },
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo(
                    ...
                    f"Metadata: {complete_config['metadata_source']}\n"
                    f"Pixel size: {self.pixel_size_um:.6f} µm/px"
                )
    ...
    def save_and_apply(self, event=None):
        ...
        try:
            ...
            if success:
                ...
                print(f"   Pixel size: {self.pixel_size_um:.6f} µm/px")
                print(f"   Source: {'Metadata' if self.has_metadata else 'Fallback'}")
                messagebox.showinfo(
                    "Success", 
    ...
    def _create_action_buttons(self, parent: ttk.Frame):
        ...
        buttons = [
            ...
            ("📋 EXPORT COMPLETE", self.export_complete_config, 20, None),
    ...
    def back(self):
        """Close the tuner and return to main menu if available"""
        if messagebox.askyesno("Confirm", "Close tuner?\n\nUnsaved changes will be lost."):
            print("🔙 Closing tuner...")
            self.root.destroy()
            
            # Try to launch main menu (fails silently if not available)
            try:
                import importlib
                pcm = importlib.import_module("pathogen_config_manager")
                print("🚀 Launching main menu...")
                if hasattr(pcm, "main"):
                    pcm.main()
                else:
                    print("ℹ️ Main menu not available. Application closed.")
            except ModuleNotFoundError:
                # Main menu module doesn't exist - that's okay
                print("ℹ️ Main menu not available. Application closed.")
            except Exception as e:
                # Some other error occurred
                print(f"⚠️ Could not launch main menu: {e}")
